        ...
        QBCore.Functions.TriggerCallback('qb-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPaletoPolice then
                    if not Config.BigBanks["paleto"]["isOpened"] then
                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                        QBCore.Functions.Progressbar("security_pass", "Valider kortet..", math.random(5000, 10000), false, true, {
                            ...
                            StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
                            QBCore.Functions.Notify("Annuleret..", "error")
                        end)
                    else
                        QBCore.Functions.Notify("Det ser ud til, at banken allerede er åbnet..", "error")
                    end
                else
                    QBCore.Functions.Notify('Min '..Config.MinimumPaletoPolice..' betjente skal være på arbejde', "error")
                end
            else
