...
function openLocker(bankId, lockerId) -- Globally Used
    ...
    if bankId == "paleto" then
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
            if hasItem then
                ...
                IsDrilling = true
                QBCore.Functions.Progressbar("open_locker_drill", "Brækker pengeskabet op..", math.random(18000, 30000), false, true, {
                    ...
                    QBCore.Functions.Notify("Vellykket!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(PlayerPedId(), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
                    TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                ...
                QBCore.Functions.Notify("Det ser ud til, at pengeskabslåsen er for stærk ..", "error")
                TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            end
        end, "drill")
    elseif bankId == "pacific" then
        ...
        IsDrilling = true
        QBCore.Functions.Progressbar("open_locker", "Brækker pengeskabet op ..", math.random(27000, 37000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
                            ...
                            if not Config.SmallBanks[closestBank]["isOpened"] then
                                QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                                    if result then
                                        ...
                                        QBCore.Functions.Progressbar("hack_gate", "Tilsutter hackerenheden ..", math.random(5000, 10000), false, true, {
                                            disableMovement = true,
                                            disableCarMovement = true,
                                            disableMouse = false,
                                            disableCombat = true,
                                ...
                                QBCore.Functions.Notify("Det ser ud til, at banken allerede er åben..", "error")
                            end
                        else
                            QBCore.Functions.Notify('Min '..Config.MinimumFleecaPolice..' betjente skal være på arbejde', "error")
                        end
                    end
                end
            else
                QBCore.Functions.Notify("Sikkerhedslåsen er aktiv, åbning af døren er i øjeblikket ikke mulig.", "error", 5500)
            end
        end)
    end
end)
