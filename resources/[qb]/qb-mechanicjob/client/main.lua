    ...
    QBCore.Functions.TriggerCallback('qb-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            QBCore.Functions.Progressbar("repair_part", "Repairing " ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                QBCore.Functions.Notify("Reparation annulleret", "error")
            end)
        else
            QBCore.Functions.Notify('Der er ikke materialer nok i opbevaringe', 'error')
        end
    end, "mechanicstash")
end
...
RegisterNetEvent('qb-vehicletuning:client:RepaireeePart', function(part)
    ...
    QBCore.Functions.Notify(""..Config.ValuesLabels[part].." Er fixet!")
end)
...
RegisterNetEvent('vehiclemod:client:getVehicleStatus', function(plate, status)
    if not (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if veh ~= nil and veh ~= 0 then
            ...
            QBCore.Functions.Notify("Du skal være i køretøjet først", "error")
        end
    else
        QBCore.Functions.Notify("Du skal være uden for køretøjet", "error")
    end
end)
...
RegisterNetEvent('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            ...
            QBCore.Functions.Notify("Du er ikke chaufføren eller på en cykel", "error")
        end
    else
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                        ...
                        QBCore.Functions.Progressbar("repair_advanced", "Reparation af køretøj", lockpickTime, false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            openingDoor = false
                            ClearPedTasks(PlayerPedId())
                            QBCore.Functions.Notify("Proces annulleret", "error")
                        end)
                    else
                ...
                if onDuty then
                    if VehicleDistance < 20 then
                        ...
                        if VehicleDistance < 1 then
                            ...
                            if InVehicle then
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, '[E] Gem bilen')
                                if IsControlJustPressed(0, 38) then
                                    DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
                                end
                            else
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, '[E] Find bilen')
                                if IsControlJustPressed(0, 38) then
                                    if IsControlJustPressed(0, 38) then
                                        VehicleList()
                                    end
                    ...
                    for k, v in pairs(Config.Plates) do
                        if v.AttachedVehicle == nil then
                            ...
                            if PlateDistance < 20 then
                                ...
                                if PlateDistance < 2 then
                                    ...
                                    if IsPedInAnyVehicle(PlayerPedId()) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z + 0.3, "[E] Placer køretøjet på platformen")
                                            if IsControlJustPressed(0, 38) then
                                                DoScreenFadeOut(150)
                                                Wait(150)
                                            ...
                                            QBCore.Functions.Notify("Du kan ikke sætte cykler på platformen!", "error")
                                        end
                                    end
                                end
                            end
                            ...
                            if PlateDistance < 3 then
                                inRange = true
                                DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Åben menuen")
                                if IsControlJustPressed(0, 38) then
                                    OpenMenu()
                                end
                            end
