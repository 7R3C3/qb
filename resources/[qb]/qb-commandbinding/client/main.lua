        ...
        if LocalPlayer.state.isLoggedIn and not keyPressed and GetLastInputMethod(0) then
            ...
            if next(keyMeta) ~= nil then
                if keyMeta[v[2]]["command"] ~= "" then
                    ...
                    QBCore.Functions.Notify('Der er stadig intet ['..v[2]..'] bundet, /binds for at binde en kommando', 'primary', 4000)
                end
            else
                QBCore.Functions.Notify('Du har ikke bundet nogen kommandoer, /binds for at binde en kommando', 'primary', 4000)
            end
        end
    end, false)
...
RegisterNUICallback('save', function(data)
    ...
    QBCore.Functions.Notify('Kommandobindinger er blevet gemt!', 'success')
