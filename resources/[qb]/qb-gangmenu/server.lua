...
AddEventHandler('qb-gangmenu:server:updateGrade', function(target, grade)
    ...
    if grade == nil then
        TriggerClientEvent('QBCore:Notify', src, {text="Bande Menu", caption="Ugyldig karakter. Kun tal, hvor 0 er den laveste karakter. Hvad er det her, amatørtime?"}, "error", 6000)
        return
    end
    if Player.PlayerData.gang.grade.level >= grade and grade <= Player.PlayerData.gang.grade.level then
        if Employee then
            if Employee.Functions.SetGang(Player.PlayerData.gang.name, grade) then
                ...
                TriggerClientEvent('QBCore:Notify', src, {text="Bande Menu", caption="Karakter ændret med succes!"}, "success")
            ...
            if result then
                ...
                if gangFinal ~= false then
                    ...
                    TriggerClientEvent('QBCore:Notify', src, {text="Gang Menu", caption="Karakter ændret med succes!"}, "success")
                    exports.oxmysql:execute('UPDATE players SET gang = ? WHERE citizenid = ?', { json.encode(gangFinal), target })
                else
                    TriggerClientEvent('QBCore:Notify', src, {text="Gang Menu", caption="Ugyldig karakter. Kun tal, hvor 0 er den laveste karakter. Hvad er det her, amatørtime?"}, "error", 6000)
                end
            else
                TriggerClientEvent('QBCore:Notify', src, {text="Gang Menu", caption="Denne person er ikke på din lønliste"}, "error", 5000)
            end
        end
    else
        ...
        TriggerClientEvent('QBCore:Notify', src, {text="Gang Menu",caption="Du er ikke autoriseret til at gøre dette. Dette er meldt til politiet."}, "error", 8000)
    end
end)
    ...
    if Employee then
        if Player.PlayerData.gang.grade.level >= Employee.PlayerData.gang.grade.level then
            if Employee.Functions.SetGang("none", '0') then
                ...
                TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source , {text="Gang Menu", caption="Du blev smidt ud fra din bande. Tænk grundigt over dine beslutninger, og du vil indse, at dette ikke er livet for dig."}, "error", 10000)
            else
                TriggerClientEvent('QBCore:Notify', src, {text="Gang Menu", caption="Send en videokupon med alle detaljer vedrørende denne fejl.."}, "error")
            end
        else
            ...
            TriggerClientEvent('QBCore:Notify', src, {text="Gang Menu",caption="Du er ikke autoriseret til at gøre dette. Dette er blevet rapporteret til " .. Player.PlayerData.gang.label .. " kasserer og regeringen."}, "error", 15000)
        end
    else
        local playerGang = '%' .. Player.PlayerData.gang.name .. '%'
        local result = exports.oxmysql:scalarSync('SELECT gang FROM players WHERE citizenid = ? AND gang LIKE ?', {target, playerGang})
        if result then
            ...
            if gangFinal ~= false then
                ...
                TriggerClientEvent('QBCore:Notify', src, {text="Gang Menu", caption="Noget fejlede. Har du prøvet at slukke og tænde den igen?"}, "error")
            end
        else
            TriggerClientEvent('QBCore:Notify', src, {text="Gang Menu", caption="Denne person er ikke på din lønliste."}, "error", 4000)
        end
