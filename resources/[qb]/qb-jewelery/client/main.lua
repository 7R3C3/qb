                ...
                if dist < 30 then
                    ...
                    if dist < 0.6 then
                        if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                            DrawText3Ds(Config.Locations[case]["coords"]["x"], Config.Locations[case]["coords"]["y"], Config.Locations[case]["coords"]["z"], '[E] Knus vitrinen')
                                ...
                                QBCore.Functions.TriggerCallback('qb-jewellery:server:getCops', function(cops)
                                    if cops >= Config.RequiredCops then
                                        if validWeapon() then
                                            ...
                                            QBCore.Functions.Notify('Dit våben er ikke stærkt nok..', 'error')
                                        end
                                    else
                                        QBCore.Functions.Notify('Ikke nok politi der mangler ('.. Config.RequiredCops ..') betjente', 'error')
                                    end
                                end)
                            end
                        end
