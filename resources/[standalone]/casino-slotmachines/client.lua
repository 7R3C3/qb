			...
			RenderScriptCams(true, 900, 900, true, false)
				...
				CreateThread(function()
					while selectedSlot ~= nil do
						...
						-- 	QBCore.Functions.TriggerCallback('BLACKJACK:server:whiteChipsAmount', function(result)
						-- 		retval = result
						-- 		exports['casinoUi']:DrawCasinoUi('show', "Avalible chips: </br> "..retval)
						-- 	end)
						-- else
		...
		for k, v in pairs(Slots) do
			...
			if dist < 2.9 then
				...
				if dist < 2.1 then
					...
					wait = 5
					inZone  = true
					text = '<b>Diamond Casino Slots</b> </p>Press [E] to sit</p>'..v.data.SlotName..'</p>'..v.data.bet..' white chip(s)'
