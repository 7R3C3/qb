    ...
    if crypto ~= nil then
        if Crypto.Worth[crypto] ~= nil then
            ...
            if NewWorth ~= nil then
                ...
                TriggerClientEvent('QBCore:Notify', src, "Du har værdien af "..Crypto.Labels[crypto].."regnet ud fra: (DKK"..Crypto.Worth[crypto].." til: Dkk"..NewWorth..") ("..ChangeLabel.." "..PercentageChange.."%)")
                ...
                TriggerClientEvent('QBCore:Notify', src, "Du angav ikke ne ny værfi .. Aktuelle værdi: "..Crypto.Worth[crypto])
            end
        else
            TriggerClientEvent('QBCore:Notify', src, "Denne krypto eksisterer ikke :(, tilgængelig: Qbit")
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "Du har ikke leveret Crypto, tilgængelig: Qbit")
    end
end, "admin")

QBCore.Commands.Add("checkcryptoworth", "", {}, false, function(source)
    local src = source
    TriggerClientEvent('QBCore:Notify', src, "Qbit har en værdi på: DKK"..Crypto.Worth["qbit"])
end)
...
QBCore.Commands.Add("crypto", "", {}, false, function(source)
    ...
    TriggerClientEvent('QBCore:Notify', src, "Du har: "..Player.PlayerData.money.crypto.."QBit, med en værdi på: DKK"..MyPocket..",-")
end)
...
RegisterServerEvent('qb-crypto:server:ExchangeFail', function()
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Cryptostick defekt", 'error')
    end
end)
...
RegisterServerEvent('qb-crypto:server:ExchangeSuccess', function(LuckChance)
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Du har byttet dit Cryptostick til: "..Amount.." QBit(\'s)", "success", 3500)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["cryptostick"], "remove")
