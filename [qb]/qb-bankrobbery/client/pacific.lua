            ...
            if not isBusy then
                if CurrentCops >= Config.MinimumPacificPolice then
                    if not Config.BigBanks["pacific"]["isOpened"] then
                        ...
                        QBCore.Functions.Progressbar("security_pass", "Bekræft venligst ..", math.random(5000, 10000), false, true, {
                    ...
                    QBCore.Functions.Notify('Min. '..Config.MinimumPacificPolice..' betjente skal være på arbejde', "error")
                end
            else
                        ...
                        if not Config.BigBanks["pacific"]["isOpened"] then
                            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                                if result then
                                    ...
                                    QBCore.Functions.Progressbar("hack_gate", "Tilsluter hackerenheden ..", math.random(5000, 10000), false, true, {
                                        disableMovement = true,
                                        disableCarMovement = true,
                                        disableMouse = false,
                                        disableCombat = true,
