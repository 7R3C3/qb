...
AddEventHandler("qb-bossmenu:server:withdrawMoney", function(amount)
    ...
    if Accounts[job] >= amount and amount > 0 then
        ...
        TriggerClientEvent('QBCore:Notify', src, {text="Boss Menu", caption="Ikke nok penge"}, 'error')
        return
    end
...
AddEventHandler('qb-bossmenu:server:updateGrade', function(target, grade)
    ...
    if grade == nil then
        TriggerClientEvent('QBCore:Notify', src, {text="Boss Menu", caption="Ugyldig karakter. Kun tal, hvor 0 er den laveste karakter. Hvad er det her, amatørtime?"}, "error", 6000)
        return
    end
    if Player.PlayerData.job.grade.level >= grade and grade <= Player.PlayerData.job.grade.level then
        if Employee then
            if Employee.Functions.SetJob(Player.PlayerData.job.name, grade) then
                ...
                TriggerClientEvent('QBCore:Notify', src, {text="Boss Menu", caption="Karakter ændret med succes!"}, "success")
                TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source, {text="Boss Menu", caption="Din nye klasse er nu [" ..grade.."]."}, "success")
            else
                TriggerClientEvent('QBCore:Notify', src, {text="Boss Menu", caption="Klasse findes ikke"}, "error")
            end
        else
            ...
            if result then
                ...
                TriggerClientEvent('QBCore:Notify', src, {text="Boss Menu", caption="Denne person er ikke på din lønliste"}, "error", 5000)
            end
        end
    else
        ...
        TriggerClientEvent('QBCore:Notify', src, {text="Boss Menu",caption="Ydu er ikke autoriseret til at gøre dette. Dette er blevet rapporteret."}, "error", 8000)
    end
end)
    ...
    if Employee then
        if Player.PlayerData.job.grade.level >= Employee.PlayerData.job.grade.level then
            if Employee.Functions.SetJob("unemployed", '0') then
                ...
                TriggerClientEvent('QBCore:Notify', src, {text="Boss Menu",caption="Du har fyret din medarbejder. Vi sender optegnelserne til " .. Player.PlayerData.job.label .. " og regeringen."}, "error", 5000)
                TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source , {text="Boss Menu", caption="Du blev fyret. Søg venligst et nyt job i retsbygningen (City Services) og afhent dine ejendele."}, "error", 10000)
            else
                TriggerClientEvent('QBCore:Notify', src, {text="Boss Menu", caption="Send en videokupon med alle detaljer vedrørende denne fejl."}, "error")
            end
        else
            ...
            TriggerClientEvent('QBCore:Notify', src, {text="Boss Menu",caption="Du er ikke autoriseret til at gøre dette. Dette er blevet rapporteret til" .. Player.PlayerData.job.label .. " og regeringen."}, "error", 15000)
        end
    else
        local playerJob = '%' .. Player.PlayerData.job.name .. '%'
        local result = exports.oxmysql:scalarSync('SELECT job FROM players WHERE citizenid = ? AND job LIKE ?', {target, playerJob})
        if result then
            ...
            if jobFinal ~= false then
                ...
                TriggerClientEvent('QBCore:Notify', src, {text="Boss Menu", caption="Noget fejlede. Har du prøvet at slukke og tænde den igen?"}, "error")
            end
        else
            TriggerClientEvent('QBCore:Notify', src, {text="Boss Menu", caption="Denne person er ikke på din lønliste."}, "error", 4000)
        end
...
AddEventHandler('qb-bossmenu:server:giveJob', function(recruit)
    ...
    if Player.PlayerData.job.isboss == true and (Target.PlayerData.job.name == 'unemployed' or Target.PlayerData.job.name == 'garbage' or Target.PlayerData.job.name == 'taxi' or Target.PlayerData.job.name == 'reporter' or Target.PlayerData.job.name == 'trucker' or Target.PlayerData.job.name == 'tow' or Target.PlayerData.job.name == 'vineyard' or Target.PlayerData.job.name == 'hotdog' or Target.PlayerData.job.name == nil) then
        if Target and Target.Functions.SetJob(Player.PlayerData.job.name, 0) then
            TriggerClientEvent('QBCore:Notify', src, {text="Boss Menu", caption="Du ansatte " .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) .. " som " .. Player.PlayerData.job.label .. ""}, "success")
            TriggerClientEvent('QBCore:Notify', Target.PlayerData.source , {text="Boss Menu", caption="Du blev ansat som " .. Player.PlayerData.job.label .. "!"}, "success")
